...
$('#'+drag_drop_parent_id).on("click", ".closePopupBtn", function(){
  var thisEditor = $(event.target).closest(".annoPopup").attr("id");
  closeEditorMenu(thisEditor);
});

$( "#"+drag_drop_parent_id).on( "click", ".polyanno-popup-min", function(event) {
  var thisEditor = $(event.target).closest(".annoPopup").attr("id");
  polyannoMinimiseEditor(thisEditor);
});

$( "#"+drag_drop_parent_id ).on( "resizestop", ".annoPopup", function( event, ui ) {
  adjustResizeBootstrapGrid($("#polyanno-page-body"), $(event.target), ui);
} );

$( "#"+drag_drop_parent_id ).on( "dragstop", ".annoPopup", function( event, ui ) {
